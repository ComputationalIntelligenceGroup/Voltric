package voltric.potential.util;
...
public class FunctionIterator {
	...
	public FunctionIterator(Function function, List<DiscreteVariable> order) {
		...
		if(order.size() < originalOrder.size())
		    throw new IllegalArgumentException("order.size() has to be >= originalOrder.size(). Order size: "+order.size() + " original order size: "+ originalOrder.size());
	...
	private void iterate(Visitor visitor, int[] states,
		...
		if (currentIteratingDimension == order.size()) {
			...
			DiscreteVariable currentIteratingVariable = order

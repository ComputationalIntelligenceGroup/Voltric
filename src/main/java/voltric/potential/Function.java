...
public class Function implements Cloneable, Potential {
    ...
    public final static Function createFunction(DiscreteData dataSet) {
        ...
        for (DiscreteDataInstance dataCase : dataSet.getInstances()) {
            // Cast the array: double[] to int[]
            int[] intValues = new int[dataSet.getVariables().size()];
            for(int i=0; i < dataCase.getNumericValues().length; i++)
                intValues[i] = (int) dataCase.getNumericValue(i);

            f._cells[f.computeIndex(intValues)] = dataSet.getWeight(dataCase);
        }
     ...
     */
    private final Function createFunction(DiscreteVariable[] variables, double[] cells,
                                          int[] magnitudes) {
        Function f = null;
